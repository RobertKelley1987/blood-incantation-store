...
function ShopByTypeDropdown({
  ...
  return (
    <ul className="font-normal flex flex-col gap-6 md:gap-2 absolute top-[60px] w-screen bg-white md:border-b left-0 p-6">
      <button
      ...
      {PRODUCT_CATEGORIES.map((category) => (
        <NavListItem category={category} handleClick={handleClick} />
      ))}
    </ul>
  );
}
